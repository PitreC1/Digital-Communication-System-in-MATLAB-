...
for i = 1:length(EbNoVec)
    ...
    alpha1 = 0.2; % Constante de escala
    t1 = 300; % Desplazamiento en el tiempo (segundos)
    ...
    Y_Ecualizado1 = (Y_Canal) - ((alpha1)*circshift(Y_Canal, [0, t1])+(alpha1^2)*circshift(Y_Canal, [0, 2*t1]));
    Y_Ecualizado2 = Y_Ecualizado1 + (alpha1^2)*circshift(Y_Canal, [0, 2*t1])+(alpha1^3)*circshift(Y_Canal, [0, 3*t1]);
    ...
    BER(i) = bits_incorrectos / numel(bits_transmitidos);
