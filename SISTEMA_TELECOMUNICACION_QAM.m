...
%Transformando Señal_TX a una matriz de 15.537 x 4  
matriz_tx = reshape(senal_tx, 2, 31074);
...
for i1 = 1:1:length(EbNoVec)
    ...
    % Transformando Señal_TX a una matriz de 15.537 x 4  
    matriz_Z = reshape(Z, 2, 31074);

%% MIMO EN RECEPCION


Y =  (chmat*matriz_tx) + (matriz_Z*1.21);

Mtz_X_Estimada = chamat_inv*Y;

X_Estimada = reshape(Mtz_X_Estimada, 1, 62148);

%% Demodulacion Pasa Banda
senal_Y1 = sqrt(2)*X_Estimada.*cos(2*pi*fc.*t);
senal_Y2 = -sqrt(2)*X_Estimada.*sin(2*pi*fc.*t);
...
if i1 == length(EbNoVec)
        ...
        input =[filtro_Y1(5000:55000)' filtro_Y2(5000:55000)'];
        eyediagram(real(input),4*fs)
...
%BER Estimada
bits_transmitidos = secuencia'; % Secuencia de bits transmitidos
bits_recibidos = secuencia_bits'; % Secuencia de bits recibidos
bits_incorrectos(i1) = sum(bits_transmitidos ~= bits_recibidos);
[~,BER(i1)]=biterr(bits_transmitidos,bits_recibidos);   
